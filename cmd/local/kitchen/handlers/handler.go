...
type TicketKafkaHandler struct {
...
var (
	_ streaming.Handler = (*TicketKafkaHandler)(nil)
)

func NewTicketKafkaHandler(logger *zap.SugaredLogger, publisher streaming.Publisher, handlersMap map[string]application.MessageHandler) *TicketKafkaHandler {
	return &TicketKafkaHandler{
		logger:      logger,
		publisher:   publisher,
...
func (h *TicketKafkaHandler) Handle(ctx context.Context, msg *kafka.Message, commitFn func() error) error {
	l := h.logger
	l.Infof("Kitchen service received message [%s]", string(msg.Value))
